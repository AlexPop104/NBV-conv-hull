...
class PcdDataset(Dataset):
    def __init__(self, root_dir,root_spherical, valid=False, folder="train", transform=default_transforms(), with_normals=False,nr_views=5,nr_points=512):
    ...
    def __preproc__(self, file, idx):
        ...
        pointcloud = torch_geometric.data.Data(normal=normals, pos=points,pos_sph=points_sph, viewstate=viewstate, target_values = target_values,next_position =  next_position, obj_class = self.classes[self.files[idx]['category']])
...
if __name__ == '__main__':
    ...
    test_dataset = PcdDataset(root,valid=True,folder=folder,nr_views=nr_views,nr_points=nr_points,root_spherical=root_spherical)
