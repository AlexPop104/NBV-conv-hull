...
class PcdDataset(Dataset):
    def __init__(self, root_dir, valid=False, folder="train", transform=default_transforms(), with_normals=False,nr_views=5,nr_points=512,list_categ=[]):
        ...
        for category in selection_classes:
                model_list = os.listdir(self.root_dir/folder/Path(category))
                for model in model_list:
                            view_list = os.listdir(self.root_dir/folder/Path(category)/Path(model))
                            for i in view_list:
                                new_dir = self.root_dir/folder/Path(category)/Path(model)/Path(i)
                                if os.path.exists(new_dir):
                                    for file in os.listdir(new_dir):
                                        if file.endswith('_acc_pc.pcd'):
                                            sample = {}
    ...
    def __preproc__(self, file, idx):
        ...
        path_viewstate = file.replace(".pcd",".npy")
        ...
        path_target_values = file.replace(".pcd",".npy")
        ...
        pointcloud = torch_geometric.data.Data(normal=normals, pos=points, viewstate=viewstate, target_values = target_values,next_position =  next_position, obj_class = self.classes[self.files[idx]['category']])
...
if __name__ == '__main__':
    ...
    test_dataset = PcdDataset(root,valid=True,folder=folder,nr_views=nr_views,nr_points=nr_points,list_categ=list_categ)
    ...
    for i in range(len(test_dataset)):
        ...
        pcd.points = o3d.utility.Vector3dVector(data.pos[0].numpy())
