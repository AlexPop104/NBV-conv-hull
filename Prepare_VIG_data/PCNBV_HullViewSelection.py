...
if __name__ == '__main__':
    ...
    path_model = "/mnt/ssd1/Alex_data/PC-NBV/Trained_models/PCNBV_VIG_33_03_08_24_12:26:02/model18.pt"
    ...
    path_export = os.path.join(root_folder,"7_NBV_hull_selected",folder_dataset)
            ...
            for model in tqdm.tqdm(model_list):
                ...
                for index_transf in range(1):
                    ...
                    for view_initial in views_select_list:
                        ...
                        # o3d.visualization.draw_geometries([pcd_net])
                        # o3d.visualization.draw_geometries([pcd_mesh_orig,pcd_all_views_selected,hull_export])
                        ...
                        pcd_hull_list.append(pcd_all_views_selected)
                        hull_score_list.append(coverage_view)

                    hull_score_list = np.array(hull_score_list)
                    select_hull = np.argmax(hull_score_list)

                    pcd_hull = pcd_hull_list[select_hull]

                    hull_export, _ = pcd_hull.compute_convex_hull()
                    hull_ls_export = o3d.geometry.LineSet.create_from_triangle_mesh(hull_export)

                    hull_ls_export.paint_uniform_color([0, 0, 1])

                    path_hull_export = os.path.join(path_export,data_type,category,model)

                    if not os.path.exists(path_hull_export):
                        os.makedirs(path_hull_export)

                    # o3d.visualization.draw_geometries([pcd_mesh_orig,pcd_hull,hull_export])

                    o3d.io.write_triangle_mesh(os.path.join(path_hull_export,"model_hull.obj"),hull_export)
                    o3d.io.write_point_cloud(os.path.join(path_hull_export,"model_hull.pcd"),pcd_hull)
